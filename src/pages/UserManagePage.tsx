...
export default function UserManagePage() {
  ...
  const loadData = useCallback(async () => {
    setLoading(true)
    try {
      const [userList, agents, candidates] = await Promise.all([
        fetchUsers(),
        fetchAgent1List(),
  ...
  const columns: ColumnsType<UserInfoWithMeta> = [
    { title: '用户名', dataIndex: 'username', width: 120, render: (v: string) => v === currentUser?.username ? <strong>{v} (当前)</strong> : v },
    { title: '角色', dataIndex: 'role', width: 120, render: (v: string) => (
      <Tag color={v === 'director' ? 'red' : v === 'agent_1' ? 'blue' : v === 'client' ? 'orange' : 'green'}>
        {ROLE_LABEL[v] || v}
      </Tag>
    )},
    { title: '上级代理', dataIndex: 'parent_id', width: 120, render: (v: string | null) => { if (!v) return '-'; const parent = users.find(u => u.id === v); return parent ? parent.username : v }},
    { title: '创建时间', dataIndex: 'created_at', width: 180, render: (v: string) => v ? new Date(v).toLocaleString('zh-CN') : '-' },
    { title: '操作', key: 'action', width: 150, render: (_: any, record: UserInfoWithMeta) => {
  ...
  return (
    <div style={{ padding: '16px 24px', background: '#f5f5f5', minHeight: '100vh' }}>
      ...
      <Table rowKey="id" columns={columns} dataSource={users} loading={loading} pagination={{ pageSize: 20, showSizeChanger: true, showTotal: (t) => '共 ' + t + ' 条' }} scroll={{ x: 'max-content' }} size="middle" />
      <Modal title={editingUser ? '编辑用户' : '新增用户'} open={modalOpen} onOk={handleSubmit} onCancel={() => { setModalOpen(false); form.resetFields() }} confirmLoading={submitting} destroyOnClose width={{ xs: '90vw', sm: 480 }}>
        <Form form={form} layout="vertical" style={{ marginTop: 16 }}>
          ...
          {selectedRole === 'agent_2' && (
            <Form.Item label={'上级经销商'} name="parent_id" rules={[{ required: true, message: '请选择上级经销商' }]}>
              <Select placeholder={'请选择上级经销商'}>{agent1List.map(a => (<Select.Option key={a.id} value={a.id}>{a.username}</Select.Option>))}</Select>
            </Form.Item>
          )}
          {selectedRole === 'client' && (
            <Form.Item label={'上级代理'} name="parent_id" rules={[{ required: true, message: '请选择上级代理' }]}>
              <Select placeholder={'请选择上级代理'}>{parentCandidates.map(a => (<Select.Option key={a.id} value={a.id}>{a.username} ({a.role === 'agent_1' ? '经销商' : '分销商'})</Select.Option>))}</Select>
            </Form.Item>
          )}
        </Form>
      </Modal>
